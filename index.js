...
if (process.env.NODE_ENV != 'test')
{
	(async () => {
		...
		await listBranches(userId, "HW4-345-repo");
		await createRepo(userId, "newrepo");
		await createIssue(userId, "HW4-345-repo", "newissue");
		await enableWikiSupport(userId, "HW4-345-repo");
...
{
	let options = getDefaultOptions("/users/repos", "POST");
	...
	return new Promise(function(resolve, reject)
	{
		request(options, function (error, response, body) {
			...
			if(statusCode == 200) {
				console.log(chalk.green('\n' + JSON.name + 'created successfully '));
			}
